...
export const Provider = ({
  ...
  const createNewItem = useCallback(() => dispatch({
    type: AppContextActions.CreateNewItem
  }), []);
  const deleteItem = useCallback((id: string) => dispatch({
    type: AppContextActions.DeleteItem,
    payload: {
      id
    }
  }), []);
  const toggleIsEditing = useCallback((id: string) => dispatch({
    type: AppContextActions.ToggleIsEditing,
    payload: {
      id
    }
  }), []);
  const editItem = useCallback((payload: IItem) => dispatch({
